...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.25rem;">
    <div style="font-family:'DM Serif Display',serif;font-size:1.4rem;color:var(--ses-gray-900);">Add New Subject</div>
...
<div class="form-card">
    <form action="{{ route('admin.subjects.store') }}" method="POST">
        ...
        <div class="row g-3">
            <div class="col-md-4"><label class="ses-label">Subject code *</label><input type="text" name="code" class="ses-input" placeholder="e.g. CS401" value="{{ old('code') }}" required></div>
            <div class="col-md-8"><label class="ses-label">Subject name *</label><input type="text" name="name" class="ses-input" placeholder="e.g. Software Engineering" value="{{ old('name') }}" required></div>
            <div class="col-md-4"><label class="ses-label">Units *</label><input type="number" name="units" class="ses-input" min="1" max="6" value="{{ old('units', 3) }}" required></div>
            <div class="col-md-8"><label class="ses-label">Schedule *</label><input type="text" name="schedule" class="ses-input" placeholder="e.g. MWF 8:00–9:00 AM" value="{{ old('schedule') }}" required></div>
            ...
            <div class="col-md-6"><label class="ses-label">Prerequisite subject</label>
                <select name="prerequisite_subject_id" class="ses-select">
                    ...
                    @foreach($subjects as $option)
                        <option value="{{ $option->id }}" {{ old('prerequisite_subject_id') == $option->id ? 'selected' : '' }}>{{ $option->code }} - {{ $option->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6"><label class="ses-label">Corequisite subject</label>
                <select name="corequisite_subject_id" class="ses-select">
                    <option value="">None</option>
                    @foreach($subjects as $option)
                        <option value="{{ $option->id }}" {{ old('corequisite_subject_id') == $option->id ? 'selected' : '' }}>{{ $option->code }} - {{ $option->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6"><label class="ses-label">Max slots *</label><input type="number" name="max_slots" class="ses-input" min="1" value="{{ old('max_slots', 40) }}" required></div>
            <div class="col-md-6"><label class="ses-label">Fee per unit (₱) *</label><input type="number" name="fee_per_unit" step="0.01" class="ses-input" value="{{ old('fee_per_unit', 800) }}" required></div>
            <div class="col-12"><label class="ses-label">Description</label><textarea name="description" class="ses-textarea" rows="2" placeholder="Optional subject description...">{{ old('description') }}</textarea></div>
            <div class="col-12">
